...
Arrangement-view clips are addressed by **start beat** (not slot index). `insert_clip_in_arrangement` calls `Track.create_midi_clip(start_beat, length)` then `set_notes` (note times are clip-relative, like the Session tools). `delete_arrangement_clip` / `duplicate_arrangement_clip` locate the clip via `_find_arrangement_clip`, which matches `clip.start_time` within `ARRANGEMENT_BEAT_EPSILON` (clips on one track can't overlap, so the match is unique). `get_arrangement_clips` is read-only (worker thread); `get_song_length` returns `Song.last_event_time`. Requires **Live 11+** (`arrangement_clips` / `create_midi_clip`); targeted at Live 12.
