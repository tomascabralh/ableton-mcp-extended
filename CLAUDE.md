...
- **Threading rule (critical):** the Live API is only safe to call from Ableton's main thread. Commands that mutate state (the list in `_process_command`: `create_midi_track`, `set_track_name`, `create_clip`, `add_notes_to_clip`, `set_clip_name`, `set_tempo`, `fire_clip`, `stop_clip`, `start_playback`, `stop_playback`, `load_browser_item`, `delete_track`, `delete_clip`, `batch`, `create_clip_with_notes`, `create_track_with_instrument`, `insert_clip_in_arrangement`, `delete_arrangement_clip`, `duplicate_arrangement_clip`, `set_arrangement_loop`, `set_arrangement_record`, `set_track_volume`, `set_track_pan`, `set_track_mute`, `set_track_solo`, `set_track_arm`, `set_send`, `set_device_parameter`, `set_clip_envelope`, `clear_clip_envelope`) are dispatched to the main thread via `self.schedule_message(0, fn)` and the worker thread blocks on a `queue.Queue` for the result with a 10s timeout. Read-only commands run directly on the worker thread. **Any new mutating command must follow the same pattern** — calling Live API directly from the socket thread will hang or crash Live.
...
- **Deferred (not built):** `delete_device`; rack/nested-device addressing (Layer 2 only walks `track.devices`, so rack-buried params under `device.chains[i].devices[j]` aren't reachable — flat native devices and rack Macro 1–8 are).

### Automation (Layer 3 — session clips only)

Two tools write parameter automation into a **session clip**: `set_clip_envelope` and `clear_clip_envelope`. Addressed by `(track_index, clip_slot_index)` + the same `target_type` ∈ `{'device','volume','pan','send'}` discriminator as the mixer/device tools (so `_resolve_envelope_parameter` reuses `_resolve_parameter`). `points` is `[[beat, value], …]` clip-relative; values use each target's unit convention (device native, volume/send dB, pan −100..100), converted on the server and clamped to `[min,max]` on the Remote Script.

- **`insert_step(t, length, value)` is the only write primitive.** `interpolation='linear'` writes **zero-length breakpoints** (`length=0.0`) — Ableton linearly interpolates between them, so **no densification is needed**. `interpolation='hold'` uses `length = next_t − t` (last point tails to `clip.length`); lengths are floored at `1e-4` so a stray out-of-order point can't hand the LOM a negative length (the server also sorts points ascending). Writes go `clip.clear_envelope(param)` → `clip.create_automation_envelope(param)` → `insert_step` loop.
- **ARRANGEMENT automation is NOT supported and is effectively impossible here.** `create_automation_envelope` raises *"Not a session clip…"* on arrangement clips — it's session-only, and the restriction is in the LiveAPI itself (Max for Live can't write arrangement automation either). The only route is real-time recording, and a spike proved a blocking remote-script handler **freezes the transport** (`current_song_time` doesn't advance), so that's deferred. See memory `ableton-arrangement-automation-impossible`.
- **No read-back** (the LOM only exposes `value_at_time` sampling, no point list). Regular tracks only. Both tools are fully wired (three places) so they're batch-compatible.
